<template>
  <div>
    <div v-if="error" class="mb-4 rounded-lg border border-red-500/50 bg-red-950/30 p-4 text-red-100" role="alert">
      <p class="font-medium">Challenge content could not be rendered</p>
    ...
    <div v-else-if="!content" class="rounded-lg border border-amber-500/50 bg-amber-950/30 p-4 text-amber-100" role="status">
      <p>No challenge content is available.</p>
    </div>
...
onMounted(() => {
  try {
    ...
    if (dataElement) {
      ...
      error.value = 'The challenge data is missing. Reload the page or return to the learning dashboard.'
    }
  } catch (e) {
    error.value = `The challenge data is invalid: ${e.message}`
  }
})
