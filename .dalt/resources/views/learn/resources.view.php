...
<main class="min-h-[calc(100vh-8rem)] bg-[#0a0d12] text-gray-300" id="app" tabindex="-1">
  <div class="mx-auto max-w-7xl px-5 py-10 sm:px-6 sm:py-16">
    <header class="max-w-2xl">
      ...
      <p class="mt-5 text-lg leading-8 text-gray-400"><?= $section === null ? 'Browse every lesson and hands-on debugging challenge at your own pace.' : htmlspecialchars($sections[$section]['description']) ?></p>
    </header>
    ...
    <nav class="mt-10 flex flex-wrap gap-2" aria-label="Resource sections">
      ...
      <?php foreach ($sections as $sectionId => $sectionMeta): ?>
        <a href="/learn/resources?section=<?= $sectionId ?>" class="rounded-lg px-3 py-2 text-sm font-medium transition-colors <?= $section === $sectionId ? 'bg-[#93DA97]/10 text-[#93DA97]' : 'text-gray-400 hover:bg-[#11161d] hover:text-gray-100' ?>"><?= htmlspecialchars($sectionMeta['title']) ?></a>
      <?php endforeach; ?>
    </nav>

    <section class="mt-12" aria-labelledby="lessons-title">
      <div class="flex items-end justify-between gap-4 border-b border-[#1e293b] pb-5"><div><h2 id="lessons-title" class="text-2xl font-bold text-gray-100">Lessons</h2><p class="mt-1 text-sm text-gray-500"><?= $section === null ? 'Read lessons in any order, or follow a learning path for a focused sequence.' : htmlspecialchars($sections[$section]['description']) ?></p></div><span class="font-mono text-xs text-gray-500"><?= count($lessons) ?> lessons</span></div>
      <div class="mt-6 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        <?php foreach ($lessons as $lesson): ?>
          <a href="/learn/lessons/<?= $lesson['id'] ?>" class="block rounded-xl border border-gray-800 bg-[#11161d] p-6 transition-all hover:border-[#93DA97]/50 hover:bg-[#161d25] group"><div class="flex items-start justify-between mb-4"><div class="text-3xl" aria-hidden="true"><?= $lesson['icon'] ?></div><div class="text-xs font-semibold text-gray-500"><?= htmlspecialchars($sections[$lesson['section']]['title']) ?> · <?= $lesson['section_order'] ?></div></div><h3 class="mb-2 text-lg font-bold text-gray-200 transition-colors group-hover:text-[#93DA97]"><?= htmlspecialchars($lesson['title']) ?></h3><p class="mb-4 text-sm text-gray-500 line-clamp-2"><?= htmlspecialchars($lesson['description']) ?></p><div class="mt-auto flex items-center text-sm font-medium text-[#93DA97]">Read lesson <span class="ml-1 transition-transform group-hover:translate-x-1" aria-hidden="true">→</span></div></a>
        <?php endforeach; ?>
      </div>
    </section>
